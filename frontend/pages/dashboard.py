...
def render_dashboard():
    ...
    with ui.column().classes("w-full p-4 md:p-6 gap-6 page-container"):
        ...
        with ui.row().classes("items-center gap-2"):
            ...
            ui.button("Refresh now", icon="refresh", on_click=lambda: refresh()).props(
                "outline"
            )
            import os
            ...
            ui.button(
                ...
                on_click=lambda: ui.navigate.to(
                    f"{API_BASE}/dashboard/export-pdf", new_tab=True
                ),
            ).props("outline")
        ...
        def render_pulse():
            ...
            color = (
                "#2F6F6B" if score >= 80 else ("#E8A33D" if score >= 50 else "#C0463C")
            )
            ...
            with pulse_container:
                ...
                with ui.row().classes("gap-6 justify-center mt-1"):
                    ui.label(f"Warehouses: {p['total_warehouses']}").classes(
                        "text-sm mono"
                    )
                    ui.label(f"Units: {p['total_units']}").classes("text-sm mono")
                    low_style = "color:#C0463C;" if p["low_stock_count"] > 0 else ""
                    ui.label(f"Low Stock: {p['low_stock_count']}").classes(
                        "text-sm mono"
                    ).style(low_style)
                    ui.label(f"Transfers: {p['recent_transfers']}").classes(
                        "text-sm mono"
                    )
        ...
        def refresh():
            ...
            cards = [
                ...
                (
                    "Inventory Value",
                    f"${data.get('total_inventory_value', 0):,.2f}",
                    "payments",
                    "text-green-600",
                ),
                (
                    ...
                    with alerts_box:
                        with ui.card().classes("w-full p-4").style(
                            ...
                            for a in alert_data.get("alerts", [])[:5]:
                                with ui.row().classes(
                                    "items-center gap-3 mt-1 text-sm"
                                ):
                                    ui.badge(
            ...
            try:
                ...
                warehouse_chart.options["xAxis"]["data"] = [
                    d.get("warehouse") for d in wh_data
                ]
                warehouse_chart.options["series"][0]["data"] = [
                    d.get("quantity") for d in wh_data
                ]
            ...
            try:
                ...
                items_chart.options["series"][0]["data"] = [
                    d.get("quantity") for d in top_data
                ]
                items_chart.update()
            except Exception:
                pass
                ...
                with activity_card:
                    ...
                    for a in activity:
                        with ui.row().classes("items-center gap-2 py-1 text-sm"):
                            ui.icon(a["icon"]).classes("text-base").style(
                                "color:var(--amber)"
                            )
