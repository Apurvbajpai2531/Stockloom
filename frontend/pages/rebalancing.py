...
def render_rebalancing():
    ...
    with ui.column().classes("w-full p-4 md:p-6 gap-4 page-container"):
        ...
        ui.label("Suggested transfers between warehouses to balance stock").classes(
            "text-sm"
        ...
        def refresh():
            ...
            if not data:
                with container:
                    ui.label(
                        "No rebalancing needed — stock is evenly distributed."
                ...
                for s in data:
                    with ui.card().classes("p-4 w-full"):
                        with ui.row().classes("items-center justify-between"):
                            ui.label(f"{s['sku']} — {s['name']}").classes(
                                "font-semibold"
                            )
                            ui.badge(
                                f"Transfer {s['suggested_transfer_qty']} units",
                                color="orange",
                            )
                        with ui.row().classes("items-center gap-2 mt-2 text-sm"):
                            ui.label(
                                f"{s['from_warehouse_name']} ({s['from_quantity']} units)"
                            )
                            ui.icon("arrow_forward")
                            ui.label(
                                f"{s['to_warehouse_name']} ({s['to_quantity']} units)"
                            )
                        ...
                        ui.button(
                            "Execute Transfer", icon="check", on_click=execute
