...
def render_cycle_count():
    ...
    with ui.column().classes("w-full p-4 md:p-6 gap-6 page-container"):
        ...
        ui.label(
            "Physical verification of stock — compare actual vs system quantities"
        ...
        def open_count(cc_id: int):
            ...
            with count_container:
                with ui.row().classes("items-center justify-between w-full"):
                    ui.label(f"Cycle Count #{cc_id} — {cc['warehouse_name']}").classes(
                        "font-semibold"
                    )
                    if cc["status"] == "open":
                        with ui.row().classes("gap-2"):
                            ui.button(
                                ...
                                on_click=lambda: complete(cc_id, False),
                            ).props("outline")
                            ui.button(
                                "Complete + Apply Adjustments",
                                icon="tune",
                                on_click=lambda: complete(cc_id, True),
                            ).props("color=primary")
                ...
                for line in cc["lines"]:
                    ...
                    with ui.card().classes("p-3 w-full").style(card_style):
                        with ui.row().classes("items-center justify-between"):
                            with ui.column():
                                ui.label(f"{line['sku']} — {line['name']}").classes(
                                    "text-sm font-semibold"
                                )
                                ui.label(f"System: {line['system_quantity']}").classes(
                                    "text-xs"
                                ).style("color:var(--ink-soft)")
                            with ui.row().classes("items-center gap-2"):
                                if line["is_verified"]:
                                    ui.badge(
                                        f"Variance: {line['variance']:+d}",
                                        color="red" if is_variance else "teal",
                                    )
                                else:
                                    counted = ui.number(
                                        "Counted Qty", value=line["system_quantity"]
                                    ).classes("w-28")
                                    ui.button(
                                        "Submit",
                                        on_click=lambda lid=line[
                                            "id"
                                        ], c=counted: submit_count(cc_id, lid, c),
                                    ).props("flat dense")

        def submit_count(cc_id: int, line_id: int, counted_input):
            try:
                result = api.post(
                    f"/cycle-counts/{cc_id}/submit-count",
                    {
                        "line_id": line_id,
                ...
                var = result["variance"]
                ui.notify(
                    f"Counted. Variance: {var:+d}",
                    type="positive" if var == 0 else "warning",
                )
                open_count(cc_id)
            except Exception as e:
                ui.notify(f"Error: {e}", type="negative")
        ...
        def complete(cc_id: int, apply: bool):
            try:
                api.post(
                    f"/cycle-counts/{cc_id}/complete",
                    {},
                    params={"apply_adjustments": str(apply).lower()},
                )
                msg = (
                    "Cycle count completed with adjustments applied!"
                    if apply
                    else "Cycle count completed."
                )
        ...
        with ui.card().classes("w-full p-4"):
            ...
            def start():
                ...
                try:
                    ...
                    ui.notify(
                        f"Cycle count started — {result['line_count']} items to count",
                        type="positive",
                    )
            ...
            ui.button("Start Count", icon="play_arrow", on_click=start).props(
                "color=primary"
            )
