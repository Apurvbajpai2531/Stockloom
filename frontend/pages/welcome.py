...
def render_welcome():
    with ui.column().classes(
        "w-full h-screen items-center justify-center gap-10 relative"
    ).style(
        ...
        with ui.column().classes("items-center gap-4 relative z-10"):
            ...
            ui.label("STOCKLOOM").classes(
                "font-bold text-5xl tracking-widest text-white"
            ).style("font-family:'Space Grotesk',sans-serif;")
            with ui.row().classes("items-center gap-2"):
                ui.element("div").style("width:32px; height:2px; background:#E8A33D;")
                ui.label("INVENTORY & WAREHOUSE CONTROL").classes(
                    "text-xs tracking-widest"
                ).style("color:#9A9C9F; font-family:'JetBrains Mono',monospace;")
                ui.element("div").style("width:32px; height:2px; background:#E8A33D;")
        ...
        ui.label("v1.0").classes("absolute bottom-6 text-xs relative z-10").style(
            "color:#5B6275;"
        )
