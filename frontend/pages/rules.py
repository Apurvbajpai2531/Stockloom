...
def render_rules():
    ...
    with ui.column().classes("w-full p-4 md:p-6 gap-6 page-container"):
        ...
        ui.label(
            "Define your own conditions — evaluated against real-time stock"
                ...
                if result["triggered_count"] > 0:
                    with triggered_box:
                        with ui.card().classes("w-full p-4").style(
                            ...
                            with ui.row().classes("items-center gap-2 mb-2"):
                                ...
                                ui.label(
                                    f"{result['triggered_count']} rule(s) currently triggered"
                                ).classes("font-semibold")
                            for t in result["triggered"]:
                                with ui.row().classes(
                                    "items-center gap-3 text-sm py-1"
                                ).style("border-bottom:1px solid var(--line);"):
                                    ui.badge(t["condition"].upper(), color="red")
                                    ui.label(f"{t['rule_name']}").classes(
                                        "font-semibold"
                                    )
                                    ui.label(
                                        f"{t['sku']} — current: {t['current_quantity']} (threshold: {t['threshold']})"
                                    ).style("color:var(--ink-soft)")
                                    ui.button(
                                        icon="open_in_new",
                                        on_click=lambda iid=t[
                                            "item_id"
                    ...
                    with triggered_box:
                        with ui.row().classes("items-center gap-2"):
                            ...
                            ui.label(
                                "All rules pass — no conditions triggered right now"
                            ).style("color:#2F6F6B;")
            except Exception as e:
                with triggered_box:
                    ui.label(f"Error evaluating rules: {e}").classes("text-red-600")

        ui.button("Evaluate Now", icon="play_arrow", on_click=check_rules).props(
            "outline"
        )
        ...
        with ui.row().classes("w-full gap-6 chart-row"):
            ...
            with ui.card().classes("flex-1 p-4"):
                ...
                def refresh_rules():
                    ...
                    if not rule_list:
                        with rules_container:
                            ui.label("No rules defined yet.").style(
                                "color:var(--ink-soft)"
                            )
                        return
                    with rules_container:
                        for r in rule_list:
                            (
                                "#C0463C"
                                if r["condition"] == "below"
                                else (
                                    "#2F6F6B"
                                    if r["condition"] == "above"
                                    else "#2563eb"
                                )
                            )
                            with ui.card().classes("p-3 w-full"):
                                with ui.row().classes("items-center justify-between"):
                                    ...
                                    with ui.row().classes("items-center gap-2"):
                                        ...
                                        ).classes("mono text-xs").style(
                                            "color:var(--ink-soft)"
                                        )
                                        ui.button(
                                            icon="delete",
                                            on_click=lambda rid=r["id"]: delete_rule(
                                                rid
                                            ),
            ...
            with ui.card().classes("flex-1 p-4"):
                ...
                item_options = {None: "All Items"} | {
                    i["id"]: f"{i['sku']} — {i['name']}" for i in items_list
                }
                wh_options = {None: "All Warehouses"} | {
                    w["id"]: w["name"] for w in warehouses_list
                }

                rule_name = ui.input("Rule Name").classes("w-full")
                item_sel = ui.select(item_options, value=None, label="Item").classes(
                    "w-full"
                )
                wh_sel = ui.select(wh_options, value=None, label="Warehouse").classes(
                    "w-full"
                )
                condition_sel = ui.select(
                    {
                        "below": "Quantity BELOW threshold",
                        "above": "Quantity ABOVE threshold",
                        "equals": "Quantity EQUALS threshold",
                    },
                    value="below",
                    label="Condition",
                ).classes("w-full")
                threshold_input = ui.number("Threshold Quantity", value=10).classes(
                    "w-full"
                )
                ...
                ui.button("Create Rule", icon="add", on_click=save_rule).classes(
                    "w-full mt-2"
