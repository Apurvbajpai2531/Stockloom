    ...
    with ui.column().classes("w-full p-4 md:p-6 gap-6 page-container"):
        ...
        with ui.row().classes("gap-4 flex-wrap"):
            with ui.card().classes("p-4 w-64"):
                ...
                ui.label("All items with current stock totals").classes(
                    "text-sm"
                ...
                ui.button(
                    ...
                    on_click=lambda: ui.navigate.to(
                        f"{API_BASE}/reports/items.csv", new_tab=True
                    ),
                ).classes("mt-3")
            ...
            with ui.card().classes("p-4 w-64"):
                ...
                ui.label("Full stock movement history").classes("text-sm").style(
                    "color:var(--ink-soft)"
                )
                ui.button(
                    "Download CSV",
                    icon="download",
                    on_click=lambda: ui.navigate.to(
                        f"{API_BASE}/reports/movements.csv", new_tab=True
                    ),
                ).classes("mt-3")
            ...
            with ui.card().classes("p-4 w-72"):
                ...
                ui.label(
                    "CSV columns: sku,name,unit,unit_price,reorder_threshold,description"
                ).classes("text-xs").style("color:var(--ink-soft)")
                ...
                def handle_upload(e):
                    ...
                    try:
                        resp = requests.post(
                            f"{API_BASE}/reports/import-items",
                            files=files,
                            headers=headers,
                        )
        ...
        valuation_chart = ui.echart(
            {
                ...
                "yAxis": {"type": "value"},
                "series": [
                    {"type": "bar", "data": [], "itemStyle": {"color": "#E8A33D"}}
                ],
        ...
        try:
            ...
            valuation_chart.options["series"][0]["data"] = [
                round(d["total_value"], 2) for d in val_data
            ]
