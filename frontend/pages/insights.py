...
def render_insights():
    ...
    with ui.column().classes("w-full p-4 md:p-6 gap-4 page-container"):
        ...
        ui.label(
            "Auto-generated, data-driven observations about your inventory"
                    ...
                    with ui.card().classes("w-full p-5").style(
                        ...
                        with ui.row().classes("items-start gap-4 w-full"):
                            with ui.element("div").classes(
                                ...
                                ui.icon(ins["icon"]).classes("text-2xl").style(
                                    f"color:{ins['color']};"
                                )

                            with ui.column().classes("flex-grow gap-1"):
                                ui.label(ins["title"]).classes(
                                    "font-semibold text-base"
                                )
                                ui.label(ins["detail"]).classes("text-sm").style(
                                    "color:var(--ink-soft)"
                                )
                            ...
                            ui.button(
                                ins["action_label"],
                                icon="arrow_forward",
                                on_click=lambda url=ins["action_url"]: ui.navigate.to(
                                    url
                                ),
                            ).props("outline").style(
                                f"border-color:{ins['color']}; color:{ins['color']};"
                            )

        ui.button("Refresh Insights", icon="refresh", on_click=refresh).classes(
            "self-end"
        )
        refresh()
