...
def render_items():
    ...
    with ui.column().classes("w-full p-4 md:p-6 gap-4 page-container"):
        ...
        # ACTIONS SLOT (KEEP OUTSIDE refresh)
        table.add_slot(
            "body-cell-actions",
            '''
            <q-td :props="props">
        ...
        # STATUS SLOT (FIXED + OUTSIDE refresh)
        table.add_slot(
            "body-cell-status",
            '''
            <q-td :props="props">
        ...
        def open_item_dialog(item: dict | None = None):
            ...
            with ui.dialog() as dialog, ui.card().classes("w-96"):
                ...
                price = ui.number(
                    "Unit Price",
                    value=float(item["unit_price"]) if is_edit else 0,
                    format="%.2f"
                )
                ...
                def validate() -> bool:
                    ...
                    if threshold.value is None or threshold.value < 0:
                        errors.append("Reorder threshold must be >= 0")
