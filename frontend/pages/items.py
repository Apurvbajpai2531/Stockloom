    ...
    with ui.column().classes("w-full p-4 md:p-6 gap-4 page-container"):
        ...
        with ui.row().classes("w-full items-center gap-4 form-row flex-wrap"):
            ...
            view_toggle = ui.button(
                icon="grid_view", on_click=lambda: switch_view()
            ).props("flat dense")
        ...
        with ui.row().classes("items-center gap-3 mt-2"):
            ...
            ui.button(icon="chevron_right", on_click=lambda: change_page(1)).props(
                "flat dense"
            )
            ...
            ui.button(
                "Add Item", icon="add", on_click=lambda: open_item_dialog()
            ).classes("ml-auto")
        ...
        grid_container = (
            ui.row().classes("w-full gap-4 flex-wrap").style("display:none;")
        )

        with ui.row().classes("items-center gap-2 mt-1"):
            ui.label("Batch:").classes("text-xs").style("color:var(--ink-soft)")
            ui.button(
                "Export Selected", icon="download", on_click=lambda: batch_export()
            ).props("flat dense outline")
            ui.button(
                "Delete Selected", icon="delete", on_click=lambda: batch_delete()
        ...
        def batch_delete():
            ...
            with ui.dialog() as confirm_dialog, ui.card():
                ui.label(
                    f"Delete {len(selected)} selected items? This cannot be undone."
                ...
                with ui.row().classes("justify-end gap-2 mt-3"):
                    ...
                    def do_delete():
                        ...
                        ui.notify(
                            f"Deleted {len(selected)-errors} items"
        ...
        def render_grid():
            ...
            with grid_container:
                for r in rows:
                    status = (
                        "LOW" if r["total_quantity"] <= r["reorder_threshold"] else "OK"
                    )
                    with ui.card().classes("w-64 p-4 cursor-pointer").on(
                        "click",
                        lambda e, item_id=r["id"]: ui.navigate.to(f"/items/{item_id}"),
                    ):
                        with ui.row().classes("items-center justify-between"):
                            ui.label(r["sku"]).classes("font-mono text-sm").style(
                                "color:var(--ink-soft)"
                            )
                            ui.badge(status, color="red" if status == "LOW" else "teal")
                        ui.label(r["name"]).classes("font-semibold mt-1")
                        ui.label(f"Qty: {r['total_quantity']}").classes(
                            "mono text-sm mt-2"
                        )
                        ui.label(
                            f"${float(r['unit_price']):.2f} / {r['unit']}"
                        ).classes("text-sm").style("color:var(--ink-soft)")

        # ---------------- REFRESH ----------------
        def refresh():
            params = {"limit": page_state["limit"], "offset": page_state["offset"]}
        ...
        def open_item_dialog(item: dict | None = None):
            ...
            with ui.dialog() as dialog, ui.card().classes("w-96"):
                ...
                ui.label("Edit Item" if is_edit else "New Item").classes(
                    "text-lg font-bold"
                )
                ...
                price = ui.number(
                    "Unit Price", value=float(item["unit_price"]) if is_edit else 0
                )
                threshold = ui.number(
                    "Reorder Threshold",
                    value=item["reorder_threshold"] if is_edit else 10,
                )
