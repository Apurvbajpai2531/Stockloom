        ...
        with ui.tab_panels(tabs, value=tab_perf).classes("w-full"):
                ...
                if not perf_data:
                    ...
                    with ui.row().classes("w-full gap-4 flex-wrap mb-4"):
                        ...
                        for label, val, icon in [
                            ...
                            with ui.card().classes("p-4 w-48").style("border-top:3px solid #E8A33D;"):
                                ui.icon(icon).style("color:#E8A33D;")
                                ui.label(label).classes("text-xs mt-1").style("color:var(--ink-soft)")
                                ui.label(str(val)).classes("text-xl font-bold mono")

                    columns = [
                        {"name": "supplier_name", "label": "Supplier", "field": "supplier_name"},
                        {"name": "item_count", "label": "Items", "field": "item_count"},
                        {"name": "inventory_value", "label": "Value Held", "field": "inventory_value"},
                        {"name": "open_purchase_orders", "label": "Open POs", "field": "open_purchase_orders"},
                        {"name": "received_purchase_orders", "label": "Received POs", "field": "received_purchase_orders"},
                        {"name": "avg_fulfillment_days", "label": "Avg Fulfillment (days)", "field": "avg_fulfillment_days"},
                        {"name": "reliability_score", "label": "Reliability", "field": "reliability_score"},
                    ]
                    for row in perf_data:
                        row["inventory_value"] = f"${row['inventory_value']:,.2f}"
                        row["avg_fulfillment_days"] = f"{row['avg_fulfillment_days']}d" if row["avg_fulfillment_days"] else "N/A"
                        row["reliability_score"] = f"{row['reliability_score']}%" if row["reliability_score"] is not None else "N/A"

                    ui.table(columns=columns, rows=perf_data, row_key="supplier_id").classes("w-full")
                ...
                if turn_data:
                    ...
                    with ui.row().classes("w-full gap-4 flex-wrap mb-4"):
                        for d in turn_data:
                            ...
                            with ui.card().classes("p-4 w-56").style(f"border-top:3px solid {color};"):
                                ui.label(d["category"]).classes("font-semibold")
                                with ui.row().classes("justify-between mt-2"):
                                    ui.label("Turnover").classes("text-xs").style("color:var(--ink-soft)")
                                    ui.label(f"{d['turnover_ratio']}x").classes("mono font-bold")
                                with ui.row().classes("justify-between"):
                                    ui.label("GMROI").classes("text-xs").style("color:var(--ink-soft)")
                                    ui.label(f"${d['gmroi']}").classes("mono font-bold").style(f"color:{color};")
                                with ui.row().classes("justify-between"):
                                    ui.label("COGS").classes("text-xs").style("color:var(--ink-soft)")
                                    ui.label(f"${d['cogs_annual']:,.0f}").classes("mono text-xs")
                                ui.badge(d["health"].upper(), color="teal" if d["health"] in ("excellent", "good") else "orange" if d["health"] == "slow" else "red").classes("mt-2")
            ...
            with ui.tab_panel(tab_dead):
                ...
                with ui.row().classes("gap-4 mb-4"):
                    with ui.card().classes("p-4 w-48").style("border-top:3px solid #C0463C;"):
                        ui.icon("inventory").style("color:#C0463C;")
                        ui.label("Dead Stock Items").classes("text-xs mt-1").style("color:var(--ink-soft)")
                        ui.label(str(dead_data["count"])).classes("text-2xl font-bold mono")
                    with ui.card().classes("p-4 w-48").style("border-top:3px solid #E8A33D;"):
                        ui.icon("payments").style("color:#E8A33D;")
                        ui.label("Capital Tied Up").classes("text-xs mt-1").style("color:var(--ink-soft)")
                        ui.label(f"${dead_data['total_tied_capital']:,.0f}").classes("text-2xl font-bold mono")
                ...
                if dead_data["items"]:
                    columns = [
                        ...
                        {"name": "tied_capital", "label": "Capital Tied", "field": "tied_capital"},
                        {"name": "days_idle", "label": "Days Idle", "field": "days_idle"},
                    ]
                    t = ui.table(columns=columns, rows=dead_data["items"], row_key="item_id").classes("w-full")
                    t.on("row-click", lambda e: ui.navigate.to(f"/items/{e.args[1]['item_id']}"))
                else:
                    ui.label("No dead stock found — great inventory health!").style("color:#2F6F6B; font-weight:600;")