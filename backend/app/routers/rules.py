...
def create_rule(payload: RuleCreate, db: Session = Depends(get_db), user: str = Depends(get_current_user)):
    if payload.condition not in ("below", "above", "equals"):
        raise HTTPException(status_code=400, detail="condition must be 'below', 'above', or 'equals'")
...
def evaluate_rules(db: Session = Depends(get_db)):
    ...
    for rule in rules:
        ...
        for item in items_to_check:
            qty_query = db.query(func.coalesce(func.sum(StockLevel.quantity), 0)).filter(
                StockLevel.item_id == item.id
            )
            if rule.warehouse_id:
                qty_query = qty_query.filter(StockLevel.warehouse_id == rule.warehouse_id)
