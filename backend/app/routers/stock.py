...
def _get_or_create_level(db: Session, item_id: int, warehouse_id: int) -> StockLevel:
    ...
    if not level:
        level = StockLevel(
            item_id=item_id, warehouse_id=warehouse_id, quantity=0, version=0
        )
        db.add(level)
        db.flush()
    return level
...
def _apply_with_lock(db: Session, level: StockLevel, delta_fn, max_retries: int = 3):
    ...
    raise HTTPException(
        status_code=409,
        detail="Stock update conflict — please retry (another change happened simultaneously)",
    )
...
def create_stock_movement(payload: StockMovementCreate, db: Session = Depends(get_db)):
    if payload.idempotency_key:
        existing = (
            ...
            .filter(StockMovement.idempotency_key == payload.idempotency_key)
            .first()
        )
        if existing:
    ...
    if payload.movement_type == MovementType.INBOUND:
        _apply_with_lock(
            db,
            source_level,
            lambda lvl: setattr(lvl, "quantity", lvl.quantity + payload.quantity),
        )

    elif payload.movement_type == MovementType.OUTBOUND:
        if source_level.quantity < payload.quantity:
            raise HTTPException(
                status_code=400, detail="Insufficient stock for outbound movement"
            )
        _apply_with_lock(
            db,
            source_level,
        ...
        if not payload.destination_warehouse_id:
            raise HTTPException(
                status_code=400,
                detail="destination_warehouse_id is required for transfers",
            )
        ...
        if source_level.quantity < payload.quantity:
            raise HTTPException(
                status_code=400, detail="Insufficient stock for transfer"
            )
        _apply_with_lock(
            db,
            source_level,
        ...
        dest_level = _get_or_create_level(
            db, payload.item_id, payload.destination_warehouse_id
        )
        _apply_with_lock(
            db,
            dest_level,
        ...
        _apply_with_lock(
            db, source_level, lambda lvl: setattr(lvl, "quantity", payload.quantity)
        )
