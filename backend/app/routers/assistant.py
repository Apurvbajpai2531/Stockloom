...
def assistant_query(payload: dict, db: Session = Depends(get_db)):
    ...
    if not text:
        return {"answer": "Ask me something like 'low stock items' or 'where is SKU-052'."}
    ...
    if any(kw in text for kw in ["low stock", "reorder", "running out", "need to order"]):
        low_items = []
        ...
        if not low_items:
            return {"answer": "Nothing is low on stock right now. All items are above their reorder threshold."}
        return {
    ...
    if sku_match or "where is" in text or "find" in text:
        search_term = sku_match.group(0).replace(" ", "-").upper() if sku_match else text.replace("where is", "").replace("find", "").strip()
        item = (
            db.query(Item)
            .filter((Item.sku.ilike(f"%{search_term}%")) | (Item.name.ilike(f"%{search_term}%")))
            .first()
        )
        if not item:
    ...
    if "total value" in text or "inventory value" in text or "worth" in text:
        ...
        return {"answer": f"Your total inventory is currently worth ${float(total_value):,.2f}."}
    ...
    if "warehouse" in text and ("how many" in text or "list" in text):
        ...
        return {
            ...
            "list": [f"{w.code} — {w.name} ({w.location or 'no location set'})" for w in warehouses],
        }
...
# Intent: forecasting / days until stockout
    if any(kw in text for kw in ["days until", "stockout", "how long", "run out"]):
        ...
        if sku_match2:
            ...
            if item:
                ...
                if velocity <= 0:
                    return {"answer": f"{item.sku} has no recent outbound movement, so I can't estimate a stockout date."}
                days = round(qty / velocity, 1)
                return {"answer": f"{item.sku} ({item.name}) has about {days} days of stock left at current usage rate."}
        return {"answer": "Tell me a SKU to check, e.g. 'how long until SKU-052 runs out'."}
    ...
    if "supplier" in text:
        ...
        if sku_match3:
            ...
            if item and item.supplier:
                return {"answer": f"{item.sku} is supplied by {item.supplier.name} ({item.supplier.contact_email or 'no email on file'})."}
            elif item:
        ...
        if "list" in text or "how many" in text:
            ...
            return {"answer": f"You have {len(suppliers)} supplier(s):", "list": [s.name for s in suppliers]}
    ...
    if "purchase order" in text or " po " in text or text.startswith("po "):
        ...
        if po_match:
            ...
            po = db.query(PurchaseOrder).filter(PurchaseOrder.po_number.ilike(f"%{po_number}%")).first()
            if po:
                return {"answer": f"PO {po.po_number} is currently '{po.status.value}' with {len(po.lines)} line item(s)."}
            return {"answer": f"I couldn't find a purchase order matching '{po_number}'."}
        open_pos = db.query(PurchaseOrder).filter(PurchaseOrder.status.in_(["draft", "ordered"])).all()
        if not open_pos:
