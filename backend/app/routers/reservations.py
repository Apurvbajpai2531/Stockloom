...
def list_reservations(status: str = "active", db: Session = Depends(get_db)):
    ...
    ).order_by(StockReservation.created_at.desc()).all()
...
def create_reservation(payload: ReservationCreate, db: Session = Depends(get_db), user: str = Depends(get_current_user)):
    # Check available stock (total - already reserved)
    total_qty = (
        db.query(func.coalesce(func.sum(StockLevel.quantity), 0))
        .filter(StockLevel.item_id == payload.item_id, StockLevel.warehouse_id == payload.warehouse_id)
        .scalar() or 0
    )
    already_reserved = (
    ...
    db.commit()
    db.refresh(res)
    return {"id": res.id, "reference": res.reference, "available_after": available - payload.quantity}
...
def fulfil_reservation(res_id: int, db: Session = Depends(get_db)):
    ...
    if res.status != "active":
        raise HTTPException(status_code=400, detail="Only active reservations can be fulfilled")
    ...
    if not level or level.quantity < res.quantity:
        raise HTTPException(status_code=400, detail="Insufficient physical stock to fulfil")
