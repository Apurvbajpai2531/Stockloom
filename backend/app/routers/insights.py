...
def generate_insights(db: Session = Depends(get_db)):
        ...
        if velocity > 0:
            ...
            if days_left <= 7:
                critical_items.append(
                    {"sku": item.sku, "name": item.name, "days": round(days_left, 1)}
                )
    ...
    dead_stock = [
        item
        for item in items
        if outbound_sums.get(item.id, 0) == 0
        and totals.get(item.id, 0) > item.reorder_threshold * 3
    ]
    if dead_stock:
        top_dead = sorted(dead_stock, key=lambda i: totals.get(i.id, 0), reverse=True)[
            :3
        ]
        insights.append(
            {
                ...
                "detail": ", ".join(
                    f"{i.sku} ({totals.get(i.id, 0)} units idle)" for i in top_dead
                ),
